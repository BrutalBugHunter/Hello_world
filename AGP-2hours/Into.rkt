    ...
    (cond ((not (null? listf))
           (send dc draw-text
                 (string-append (number->string counter) " ."
                                "(" (number->string (round-of (caar listf)))
                                "," (number->string (round-of (cdar listf)))
                                ")")
...
(define (round-of a)
  (exact->inexact (/ (truncate (* 100 (exact->inexact a))) 100)))
...
(define (polygons s)
  ...
  (define (orthogonal dc vertices-list mouse-pos-vertex a)
    ...
    (let* ((x1 (send mouse-pos-vertex get-x))
           ...
           (expected-x1  (cond ((and (not (eq? slope-perp-line 'infinity))
                                     (not (eq? slope-perp-line 0)))
                                (+ x2 (/ (- y1 y2) slope-perp-line))))))
         
      (cond
        
        ((and (not (eq? slope-perp-line 'infinity))
              (not (eq? slope-perp-line 0))
              (< (abs (- y1 expected-y1)) a))
         (begin (send dc draw-line x2 y2 x1 expected-y1)
                (set! temporary-list (cons x1 expected-y1))
                (send dc draw-text
                      (string-append "("
                                     (number->string (round-of x1))
                                     ","
                                     (number->string (round-of (- 900 expected-y1)))
                                     ")")
                      x1 expected-y1)))
        
        ((and (not (eq? slope-perp-line 'infinity))
              (not (eq? slope-perp-line 0))
              (< (abs (- x1 expected-x1)) a))
         (begin (send dc draw-line x2 y2 expected-x1 y1)
                (set! temporary-list (cons expected-x1 y1))
                (send dc draw-text
                      (string-append "("
                                     (number->string (round-of expected-x1))
                                     ","
                                     (number->string (round-of (- 900 y1)))
                                     ")")
                      expected-x1 y1)))
    ...
    (let* ((slope-line1  (cond ((eq? (- (cdar (reverse list-vertices)) (cdadr (reverse list-vertices))) 0)
      ...
      (cond ((eq? x 'no-x)
             (begin (send dc draw-line (car point1) (cdr point1) mouse-x mouse-y)
                    ...
                    (send dc draw-text
                          (string-append "("
                                         (number->string (round-of mouse-x))
                                         ","
                                         (number->string (round-of (- 900 mouse-y)))
                                         ")")
                          mouse-x mouse-y)))
            ...
            ((and (< (abs (- x mouse-x)) a ) (< (abs (- y mouse-y)) a ))
             (begin (send dc draw-line (car point1) (cdr point1) x y)
                    ...
                    (send dc draw-text
                          (string-append "("
                                         (number->string (round-of x))
                                         ","
                                         (number->string (round-of (- 900 y)))
                                         ")")
                          x y)))
                                  ...
                                  (if (= counter 2)
                                      (begin (send dc draw-line
                                             ...
                                             (send dc draw-text
                                                   (string-append "("
                                                                  (number->string
                                                                   (round-of (send event get-x)))
                                                                  ","
                                                                  (number->string (round-of (- 900 (send event get-y))))
                                                                  ")")
